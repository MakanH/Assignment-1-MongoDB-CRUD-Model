...
createServer(async function (request, response) {
    ...
    const firstRef = await model.addReflection("3rd reflection added", 3, "2026-05-31", 50);
    ...
    response.write("\nUPDATE SINGLE REFLECTION");
    await model.updateSingleReflection("2026-05-30", "This is the updated version", 1, 30);
    const updatedRef = await model.getSingleReflection("2026-05-30");
    response.write(`Reflection on date ${updatedRef.date} has been changed to ${updatedRef.reflectionText}`);
    response.end("");
}).listen(port, () => {
